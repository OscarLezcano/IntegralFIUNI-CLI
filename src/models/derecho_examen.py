...
class DerechoExamen:
    def __init__(self, id_examenes, materia, porcentaje_pp, porcentaje_asistencia, fecha_examen, fecha_limite_inscripcion, fecha_inscripcion, estado):
        ...
        self.fecha_inscripcion = self.formatear_fecha(fecha_inscripcion)
        self.estado = estado

    def formatear_fecha(self, date_str, incluir_hora=False):
        if date_str == "01/01/0001":
            return "No Inscripto" 
        elif incluir_hora:
            return datetime.strptime(date_str[:16], "%Y-%m-%dT%H:%M").strftime("%d/%m/%Y %H:%M")
        else:
